...
A Flutter-based e-wallet application with AI-driven credit insights, digital microloans (Spark Loan), and AI-powered eKYC identity verification. Built with a serverless AWS backend.
...
graph TB
    ...
    UI -->|HTTP POST| APIGW
    State -->|File Upload| S3
    APIGW --> GEN
    APIGW --> VERIFY
    APIGW --> SUBMIT
    APIGW --> AI
    GEN -->|Presigned URL| S3
    VERIFY --> REK
    VERIFY --> TEX
    VERIFY --> DDB
    SUBMIT --> DDB
    AI --> BED
    BED -->|Credit Analysis| AI
```
